...
function ChildView(props) {
  ...
  return (
    <div className="childView">
      ...
      {isOpen ? <div className="adding-newRecord">
        <h2>Adding New Record</h2>
        <form onSubmit={onSubmit}>
          <div>
            <label htmlFor="weight" >Weight</label>
            <input id="weight" type="text" name="weight" placeholder="weight" onChange={onChange} />
          </div>
          <div>
            <label htmlFor="height" >Height</label>
            <input id="height" type="text" name="height" placeholder="height" onChange={onChange} />
          </div>
          <div id="adding-des">
            <label htmlFor="description" >Description</label>
            <textarea id="description" type="text" name="description" placeholder="type here .." onChange={onChange} />
          </div>
      ...
      </div> : <div className="addrecord">
          <Link to={`${childId}`} onClick={() => setIsOpen(true)}>add Record</Link>
        </div>}


      <div className="childTrack">
        {viewAll ? props.childTrack.map((data, index) => (
          <div className="record-container">
            ...
            {index == props.childTrack.length - 1 ? <p className="viewallrecords" onClick={() => setViewAll(false)}>View only last record</p> : null}
          </div>
...
export default connect(mapStatetoProps, { userInfo, getChildRecord, addingChildRecord })(ChildView);
